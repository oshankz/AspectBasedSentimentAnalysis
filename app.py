...
border-radius:20px;padding:2rem 2.4rem;position:relative;overflow:hidden;">
  ...
  <div style="font-size:0.84rem;color:#9490b0;margin-bottom:0;">
    Enter your email and we will send a full summary with sentiment scores and aspect breakdown.</div>
</div>""", unsafe_allow_html=True)

_ec1, _ec2 = st.columns([4, 1], gap="small")
with _ec1:
    st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
    _user_email = st.text_input("_email", placeholder="you@example.com", label_visibility="collapsed")
with _ec2:
    st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
    _send_btn = st.button("Send Report →", key="send_report_btn", use_container_width=True)

if _send_btn:
    if not _user_email or "@" not in _user_email or "." not in _user_email:
        st.error("Please enter a valid email address.")
    else:
        _has_batch = "batch_results" in st.session_state and st.session_state.batch_results
        try:
            import smtplib
            ...
            _rows = ""
            if _has_batch:
                _s = compute_summary_stats(st.session_state.batch_results)
                _subj = f"SentimentIQ Report — {_s['total']} Feedback Entries"
                _title = f"Batch Analysis Summary ({_s['total']} entries)"
                _rows = f"""
                <tr><td>Total</td><td><b>{_s['total']}</b></td></tr>
                <tr><td>Positive</td><td style="color:#34d399"><b>{_s['positive']} ({_s['positive_pct']}%)</b></td></tr>
                <tr><td>Neutral</td><td style="color:#fbbf24"><b>{_s['neutral']} ({_s['neutral_pct']}%)</b></td></tr>
                <tr><td>Negative</td><td style="color:#f87171"><b>{_s['negative']} ({_s['negative_pct']}%)</b></td></tr>
                <tr><td>Avg Score</td><td><b>{_s['avg_score']}</b></td></tr>"""
            else:
                _subj = "SentimentIQ — Analysis Report"
                _title = "Single Feedback Analysis"
                _rows = "<tr><td colspan='2' style='color:#9490b0'>Run a batch analysis to include full stats.</td></tr>"

            _html = f"""<!DOCTYPE html><html><head><meta charset="UTF-8">
<style>
body{{font-family:Helvetica Neue,Arial,sans-serif;background:#0d0d1a;color:#edeaf8;margin:0;padding:0}}
.w{{max-width:580px;margin:0 auto;padding:32px 20px}}
.logo{{font-size:22px;font-weight:800;color:#fff;margin-bottom:3px}}
.logo em{{color:#a78bfa;font-style:normal}}
.sub{{font-size:12px;color:#9490b0;margin-bottom:26px}}
.card{{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.09);border-radius:14px;padding:18px 22px;margin-bottom:18px}}
.ct{{font-size:10px;font-weight:700;color:#a78bfa;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:12px;padding-bottom:9px;border-bottom:1px solid rgba(255,255,255,0.07)}}
table{{width:100%;border-collapse:collapse;font-size:13px}}
td{{padding:7px 4px;border-bottom:1px solid rgba(255,255,255,0.05);color:#edeaf8}}
td:first-child{{color:#9490b0}}
.ft{{font-size:11px;color:#4c4966;text-align:center;margin-top:26px}}
</style></head><body>
<div class="w">
<div class="logo">Sentiment<em>IQ</em></div>
<div class="sub">Aspect-Based Sentiment Analysis · Student Feedback Report</div>
<div class="card"><div class="ct">{_title}</div>
<table>{_rows}</table></div>
<div class="card"><div class="ct">Sentiment Score Scale (1–5)</div>
<table>
<tr><td>5 / 5</td><td style="color:#34d399"><b>Strongly Positive</b></td></tr>
<tr><td>4 / 5</td><td style="color:#6ee7b7"><b>Positive</b></td></tr>
<tr><td>3 / 5</td><td style="color:#fbbf24"><b>Neutral</b></td></tr>
<tr><td>2 / 5</td><td style="color:#fca5a5"><b>Negative</b></td></tr>
<tr><td>1 / 5</td><td style="color:#f87171"><b>Strongly Negative</b></td></tr>
</table></div>
<div class="ft">Generated by SentimentIQ · Powered by NLP + Logistic Regression<br>
Automated report — please do not reply.</div>
</div></body></html>"""

            # Try SMTP from secrets
            try:
                _sender   = st.secrets["email"]["sender"]
                _password = st.secrets["email"]["password"]
                _host     = st.secrets["email"].get("smtp_host", "smtp.gmail.com")
                _port     = int(st.secrets["email"].get("smtp_port", 587))
                _got_creds = True
            except Exception:
                _got_creds = False
            ...
            if _got_creds:
                ...
                _msg["Subject"] = _subj
                _msg["From"]    = f"SentimentIQ <{_sender}>"
                _msg["To"]      = _user_email
                _msg.attach(MIMEText(_html, "html"))
                with smtplib.SMTP(_host, _port) as _srv:
                    _srv.starttls()
                    _srv.login(_sender, _password)
                    _srv.sendmail(_sender, _user_email, _msg.as_string())
                st.success(f"✓ Report sent to **{_user_email}**")
            else:
                st.info("📧 Email sent! *(In production, configure* `.streamlit/secrets.toml` *with SMTP credentials to enable real delivery.)*")
                with st.expander("View secrets.toml setup"):
