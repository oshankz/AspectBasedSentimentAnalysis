...
def sent_to_scale(confidence: float, sentiment: str) -> int:
    """1=Strongly Negative, 2=Negative, 3=Neutral, 4=Positive, 5=Strongly Positive"""
    if sentiment == "Neutral":
        return 3
    elif sentiment == "Positive":
        return 5 if confidence >= 0.70 else 4
    else:
        return 1 if confidence >= 0.70 else 2
...
@media (max-width:768px){
  .hero-h1{
    left:0;
    white-space:normal;   /* important so text wraps */
  }
}
.hero-words {
...
@media (max-width:768px) {
  ...
  .hero-tag  { font-size:0.4rem !important; white-space:normal !important; text-align:center; }
  .hero-h1   { font-size:clamp(1.3rem,5.5vw,2.2rem) !important; white-space:normal !important; }
  .hero-words em { font-size:clamp(1rem,4.5vw,1.5rem) !important; }
  .strip     { flex-wrap:wrap !important; gap:0.5rem !important; padding:0.8rem !important; margin:1.5rem 0 0 !important; }
  .strip-item{ flex:0 0 calc(50% - 0.25rem) !important; min-width:0 !important; max-width:none !important; border-radius:10px !important; padding:1rem 0.5rem !important; }
  /* Hide subtitle tagline on inner pages */
  .pghdr-sub { display:none !important; }
  /* Result tiles: 2x2 grid */
  .rgrid { grid-template-columns:1fr 1fr !important; gap:0.6rem !important; }
  .rtile { padding:0.9rem 0.5rem !important; }
  .rtile-val { font-size:1.4rem !important; }
  .rtile-lbl { font-size:0.55rem !important; }
  /* Batch info grid: side-by-side → stacked */
  .bgrid { grid-template-columns:1fr !important; gap:0.8rem !important; }
}
...
# EMAIL HELPERS
# =============================================================================
_SENDER_EMAIL = "oshanksaraswat@gmail.com"
_APP_PASSWORD  = "zfbw tusi ezzn tgxd"

_EMAIL_CSS = """<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:Helvetica Neue,Arial,sans-serif;background:#05050d;color:#edeaf8}
.wrap{max-width:620px;margin:0 auto;padding:36px 20px}
.logo{font-size:22px;font-weight:800;color:#fff;letter-spacing:-0.04em;margin-bottom:3px}
.logo em{color:#a78bfa;font-style:normal}
.sub{font-size:11px;color:#9490b0;margin-bottom:28px}
.card{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:18px 20px;margin-bottom:14px;position:relative;overflow:hidden}
.card::before{content:\'\';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(167,139,250,0.4),transparent)}
.ct{font-size:9px;font-weight:700;color:#a78bfa;letter-spacing:0.14em;text-transform:uppercase;padding-bottom:10px;border-bottom:1px solid rgba(255,255,255,0.07);margin-bottom:14px}
table{width:100%;border-collapse:collapse;font-size:13px}
td{padding:8px 5px;border-bottom:1px solid rgba(255,255,255,0.05);color:#edeaf8;vertical-align:middle}
td:first-child{color:#9490b0;width:36%}
.badge{display:inline-block;padding:3px 10px;border-radius:50px;font-size:11px;font-weight:700}
.pos{background:rgba(52,211,153,0.12);color:#34d399;border:1px solid rgba(52,211,153,0.25)}
.neu{background:rgba(251,191,36,0.12);color:#fbbf24;border:1px solid rgba(251,191,36,0.25)}
.neg{background:rgba(248,113,113,0.12);color:#f87171;border:1px solid rgba(248,113,113,0.25)}
.arow{display:flex;align-items:center;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.05)}
.arow:last-child{border-bottom:none}
.aname{font-size:13px;font-weight:600;min-width:110px}
.abar-bg{flex:1;height:3px;background:rgba(255,255,255,0.06);border-radius:2px;margin:0 10px}
.ft{font-size:10px;color:#4c4966;text-align:center;margin-top:24px;line-height:1.8}
</style>"""

def _badge_cls(sent):
    return {"Positive":"pos","Negative":"neg","Neutral":"neu"}.get(sent,"neu")

def _sc_color(sc):
    return {1:"#f87171",2:"#fca5a5",3:"#fbbf24",4:"#6ee7b7",5:"#34d399"}.get(sc,"#9490b0")

def _sc_label(sc):
    return {1:"Strongly Negative",2:"Negative",3:"Neutral",4:"Positive",5:"Strongly Positive"}.get(sc,"")

def build_single_html(result):
    sc       = sent_to_scale(result["confidence"], result["sentiment"])
    sc_clr   = _sc_color(sc)
    sc_lbl   = _sc_label(sc)
    smap     = {"Positive":"#34d399","Neutral":"#fbbf24","Negative":"#f87171"}
    sent_clr = smap.get(result["sentiment"], "#a78bfa")
    ...
    def card_html(title, inner):
        return (
            "<div style='background:" + CARD + ";border:1px solid " + BORD + ";"
            "border-radius:12px;padding:22px 24px;margin-bottom:16px'>"
            "<div style='font-size:9px;font-weight:700;color:#a78bfa;letter-spacing:0.13em;"
            "text-transform:uppercase;padding-bottom:10px;"
            "border-bottom:1px solid " + BORD + ";margin-bottom:16px'>" + title + "</div>"
            + inner +
            "</div>"
        )

    def trow(label, val, last=False):
        bb = "" if last else "border-bottom:1px solid " + BORD + ";"
        return (
            "<tr>"
            "<td style='color:" + MUTED + ";font-size:13px;padding:10px 0;" + bb + "width:36%'>" + label + "</td>"
            "<td style='font-size:13px;padding:10px 0 10px 14px;" + bb + "color:" + TEXT + "'>" + val + "</td>"
            "</tr>"
        )

    def badge(sent):
        c = smap.get(sent, "#a78bfa")
        return (
            "<span style='display:inline-block;padding:4px 14px;border-radius:50px;"
            "font-size:12px;font-weight:700;color:" + c + ";"
            "border:1px solid " + c + "44;background:" + c + "18'>" + sent + "</span>"
        )

    def pbar(label, val):
        c   = smap.get(label, "#a78bfa")
        pct = round(val * 100, 1)
        ...
        return (
            ...
            "<span style='font-size:13px;font-weight:700;color:" + c + "'>" + str(pct) + "%</span></div>"
            "<div style='background:rgba(255,255,255,0.07);border-radius:6px;height:8px'>"
            "<div style='height:8px;width:" + str(w) + "%;background:" + c + ";border-radius:6px'></div>"
            "</div></div>"
        )
    ...
    for ar in result["aspect_results"]:
        ar_sc  = sent_to_scale(ar["confidence"], ar["sentiment"])
        ar_clr = _sc_color(ar_sc)
        pct    = min(int(ar["confidence"] * 100), 100)
        asp_rows.append(
            "<tr style='border-bottom:1px solid " + BORD + "'>"
            "<td style='padding:12px 0;color:" + TEXT + ";font-size:13px;font-weight:600'>"
            + ar["emoji"] + "&nbsp;" + esc(ar["aspect"]) + "</td>"
            "<td style='padding:12px 8px;text-align:center'>" + badge(ar["sentiment"]) + "</td>"
            "<td style='padding:12px 8px;text-align:center;font-size:15px;font-weight:800;color:" + ar_clr + "'>"
            + str(ar_sc) + "/5</td>"
            "<td style='padding:12px 0 12px 8px;width:100px'>"
            "<div style='background:rgba(255,255,255,0.07);border-radius:4px;height:6px'>"
            "<div style='height:6px;width:" + str(pct) + "%;background:" + ar_clr + ";border-radius:4px'></div>"
            "</div></td></tr>"
        )

    asp_inner = (
        "<table style='width:100%;border-collapse:collapse'>"
        "<tr style='font-size:9px;color:" + MUTED + ";border-bottom:1px solid " + BORD + "'>"
        "<td style='padding:6px 0'>Aspect</td>"
        "<td style='padding:6px 8px;text-align:center'>Sentiment</td>"
        "<td style='padding:6px 8px;text-align:center'>Score</td>"
        "<td style='padding:6px 0 6px 8px'>Confidence</td></tr>"
        + "".join(asp_rows) + "</table>"
    ) if asp_rows else "<p style='color:" + MUTED + ";font-size:13px'>No specific aspects detected.</p>"

    # Probability bars
    prob_inner = "".join(pbar(k, v) for k, v in result["probabilities"].items())

    # Scale
    scale_labels = {1:"Strongly<br>Negative", 2:"Negative", 3:"Neutral", 4:"Positive", 5:"Strongly<br>Positive"}
    scale_cells  = []
    for i in [1, 2, 3, 4, 5]:
        active  = (i == sc)
        bg      = (_sc_color(i) + "28") if active else "rgba(255,255,255,0.04)"
        border  = ("2px solid " + _sc_color(i)) if active else ("1px solid " + BORD)
        scale_cells.append(
            "<td style='padding:0 4px;width:20%'>"
            "<div style='background:" + bg + ";border:" + border + ";border-radius:10px;padding:12px 4px;text-align:center'>"
            "<div style='font-size:15px;font-weight:800;color:" + _sc_color(i) + "'>" + str(i) + "</div>"
            "<div style='font-size:9px;color:" + _sc_color(i) + ";margin-top:4px;line-height:1.4'>" + scale_labels[i] + "</div>"
            "</div></td>"
        )

    # Download buttons section (embedded in email)
    dl_btn = (
        "<a href='#' style='display:inline-block;padding:10px 20px;margin-right:10px;"
        "background:rgba(124,58,237,0.18);color:#a78bfa;"
        "border:1px solid rgba(124,58,237,0.45);border-radius:8px;"
        "font-size:12px;font-weight:700;text-decoration:none'>"
        "&#8659;&nbsp;Open this email to download</a>"
    )

    parts = [
        "<!DOCTYPE html><html><head><meta charset='UTF-8'>",
        "<style>*{box-sizing:border-box;margin:0;padding:0}"
        "body{font-family:-apple-system,Helvetica Neue,Arial,sans-serif;background:" + BG + ";color:" + TEXT + "}"
        ...
        "<div class='wrap' style='max-width:600px;margin:0 auto;padding:36px 20px'>",

        # Logo
        "<div style='margin-bottom:4px'>"
        "<span style='font-size:26px;font-weight:800;color:#fff;letter-spacing:-0.04em'>Sentiment</span>"
        "<span style='font-size:26px;font-weight:800;color:#a78bfa;letter-spacing:-0.04em'>IQ</span></div>",
        "<div style='font-size:12px;color:" + MUTED + ";margin-bottom:32px'>Single Feedback Analysis Report</div>",

        # Feedback text
        card_html("Analysed Feedback",
            "<div style='font-size:14px;color:" + MUTED + ";line-height:1.85;"
            "border-left:3px solid #7c3aed;padding-left:16px;font-style:italic'>"
            + esc(result["original_text"]) + "</div>"),

        # Overall result
        card_html("Overall Result",
            "<table style='width:100%'>"
            + trow("Overall Sentiment",
                "<span style='display:inline-block;padding:4px 14px;border-radius:50px;"
                "font-size:13px;font-weight:700;color:" + sent_clr + ";"
                "border:1px solid " + sent_clr + "44;background:" + sent_clr + "18'>"
                + result["emoji"] + "&nbsp;" + result["sentiment"] + "</span>")
            + trow("Sentiment Score",
                "<span style='font-size:20px;font-weight:800;color:" + sc_clr + "'>" + str(sc) + " / 5</span>"
                "&ensp;<span style='font-size:12px;color:" + MUTED + "'>(" + sc_lbl + ")</span>")
            + trow("Detected Aspects",
                "<span style='color:#a78bfa'>" + (", ".join(result["aspects"]) if result["aspects"] else "—") + "</span>",
                last=True)
            + "</table>"),

        # Aspect breakdown
        card_html("Aspect-wise Breakdown", asp_inner),

        # Probability bars
        card_html("Probability Distribution", prob_inner),

        # Scale
        card_html("Sentiment Score Scale",
            "<table style='width:100%'><tr>" + "".join(scale_cells) + "</tr></table>"),

        # Footer
        "<div style='font-size:11px;color:" + MUTED + ";text-align:center;margin-top:28px;line-height:2'>"
        "Generated by SentimentIQ &middot; Powered by NLP + Logistic Regression<br>"
        "This is an automated report &mdash; please do not reply.</div>",

        "</div></body></html>",
    ]
    return "".join(parts)

def build_batch_html(results, stats, file_bytes=None, file_name=None):
    pos_pct = stats["positive_pct"]; neu_pct = stats["neutral_pct"]; neg_pct = stats["negative_pct"]
    smap    = {"Positive":"#34d399","Neutral":"#fbbf24","Negative":"#f87171"}
    ...
    def trow(label, val, last=False):
        ...
        return (
            "<tr>"
            "<td style='color:" + MUTED + ";font-size:13px;padding:10px 0;" + bb + "width:38%'>" + label + "</td>"
            "<td style='font-size:13px;padding:10px 0 10px 14px;" + bb + "color:" + TEXT + "'>" + val + "</td>"
            "</tr>"
        )
    ...
    def badge(sent):
        c = smap.get(sent, "#a78bfa")
        return (
            "<span style='display:inline-block;padding:3px 10px;border-radius:50px;"
    ...
    # Distribution bar
    dist_html = (
        "<div style='border-radius:8px;overflow:hidden;height:20px;display:flex;margin-bottom:12px'>"
        "<div style='width:" + str(pos_pct) + "%;background:#34d399'></div>"
        "<div style='width:" + str(neu_pct) + "%;background:#fbbf24'></div>"
        "<div style='width:" + str(neg_pct) + "%;background:#f87171'></div>"
        "</div>"
        "<div style='display:flex;gap:20px;font-size:12px'>"
        "<span style='color:#34d399'>&#9632;&nbsp;Positive " + str(pos_pct) + "%</span>"
        "<span style='color:#fbbf24'>&#9632;&nbsp;Neutral " + str(neu_pct) + "%</span>"
        "<span style='color:#f87171'>&#9632;&nbsp;Negative " + str(neg_pct) + "%</span>"
        "</div>"
    )

    # Aspect bars
    asp_parts = []
    for asp, counts in stats["aspect_sentiment"].items():
        tot = sum(counts.values()) or 1
        p = int(counts.get("Positive", 0) / tot * 100)
        n = int(counts.get("Neutral",  0) / tot * 100)
        g = int(counts.get("Negative", 0) / tot * 100)
        asp_parts.append(
            "<div style='margin-bottom:14px'>"
            "<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:6px'>"
            "<span style='font-size:13px;font-weight:600;color:" + TEXT + "'>" + esc(asp) + "</span>"
            "<span style='font-size:11px'>"
            "<span style='color:#34d399'>+" + str(p) + "%</span>&ensp;"
            "<span style='color:#fbbf24'>~" + str(n) + "%</span>&ensp;"
            "<span style='color:#f87171'>-" + str(g) + "%</span>"
            "</span></div>"
            "<div style='background:rgba(255,255,255,0.07);border-radius:6px;height:10px;display:flex;overflow:hidden'>"
            "<div style='width:" + str(p) + "%;background:#34d399'></div>"
            "<div style='width:" + str(n) + "%;background:#fbbf24'></div>"
            "<div style='width:" + str(g) + "%;background:#f87171'></div>"
            "</div></div>"
        )
    ...
    for i, r in enumerate(results):
        sc     = sent_to_scale(r["confidence"], r["sentiment"])
        clr    = _sc_color(sc)
        ...
        entry_rows.append(
            ...
            "<td style='padding:9px 8px;font-size:11px;color:" + MUTED + ";text-align:center;width:32px'>" + str(i+1) + "</td>"
            "<td style='padding:9px 8px;font-size:12px;color:" + TEXT + ";line-height:1.5'>" + esc(r["original_text"]) + "</td>"
            "<td style='padding:9px 8px;text-align:center;white-space:nowrap'>" + badge(r["sentiment"]) + "</td>"
            "<td style='padding:9px 8px;text-align:center;font-size:13px;font-weight:800;color:" + clr + "'>" + str(sc) + "/5</td>"
            "<td style='padding:9px 8px;font-size:11px;color:#a78bfa'>" + esc(", ".join(r["aspects"]) if r["aspects"] else "—") + "</td>"
            "</tr>"
        )
    ...
    entries_inner = (
        ...
        "<thead><tr style='font-size:9px;color:" + MUTED + ";border-bottom:1px solid " + BORD + "'>"
        "<td style='padding:6px 8px'>#</td>"
        "<td style='padding:6px 8px'>Feedback</td>"
        "<td style='padding:6px 8px;text-align:center'>Sentiment</td>"
        "<td style='padding:6px 8px;text-align:center'>Score</td>"
        "<td style='padding:6px 8px'>Aspects</td></tr></thead>"
        "<tbody>" + "".join(entry_rows) + "</tbody></table></div>"
    )
    ...
    for i in [1, 2, 3, 4, 5]:
        scale_cells.append(
            "<td style='padding:0 4px;width:20%'>"
            "<div style='background:rgba(255,255,255,0.04);border:1px solid " + BORD + ";"
    ...
    # File download (base64 embedded)
    file_section = ""
    if file_bytes and file_name:
        import base64
        b64  = base64.b64encode(file_bytes).decode()
        mime = "text/csv" if str(file_name).lower().endswith(".csv") else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        file_section = (
            "<div style='background:" + CARD + ";border:1px solid rgba(124,58,237,0.28);"
            "border-radius:12px;padding:18px 22px;margin-bottom:16px;"
            "display:flex;align-items:center;gap:16px'>"
            "<div style='font-size:26px'>&#128196;</div>"
            "<div style='flex:1'>"
            "<div style='font-size:14px;font-weight:700;color:" + TEXT + ";margin-bottom:3px'>" + esc(str(file_name)) + "</div>"
            "<div style='font-size:12px;color:" + MUTED + "'>" + str(len(results)) + " feedback entries</div></div>"
            "<a href='data:" + mime + ";base64," + b64 + "' download='" + esc(str(file_name)) + "' "
            "style='display:inline-block;padding:8px 16px;background:rgba(124,58,237,0.18);"
            "color:#a78bfa;border:1px solid rgba(124,58,237,0.45);border-radius:8px;"
    ...
    parts = [
        ...
        "</style></head>",
        "<body style='background:" + BG + ";margin:0;padding:0'>",
        "<div class='wrap' style='max-width:640px;margin:0 auto;padding:36px 20px'>",

        "<div style='margin-bottom:4px'>"
        "<span style='font-size:26px;font-weight:800;color:#fff;letter-spacing:-0.04em'>Sentiment</span>"
        "<span style='font-size:26px;font-weight:800;color:#a78bfa;letter-spacing:-0.04em'>IQ</span></div>",
        "<div style='font-size:12px;color:" + MUTED + ";margin-bottom:32px'>"
        "Batch Analysis Report &middot; " + str(stats["total"]) + " entries</div>",

        file_section,

        card_html("Summary",
            "<table style='width:100%'>"
            + trow("Total Feedback", "<b style='font-size:18px;color:" + TEXT + "'>" + str(stats["total"]) + "</b>")
            + trow("Positive",  "<b style='color:#34d399'>" + str(stats["positive"])  + " (" + str(pos_pct) + "%)</b>")
            + trow("Neutral",   "<b style='color:#fbbf24'>" + str(stats["neutral"])   + " (" + str(neu_pct) + "%)</b>")
            + trow("Negative",  "<b style='color:#f87171'>" + str(stats["negative"])  + " (" + str(neg_pct) + "%)</b>", last=True)
            + "</table>"),

        card_html("Sentiment Distribution", dist_html),

        card_html("Aspect-wise Sentiment", "".join(asp_parts)) if asp_parts else "",

        card_html("All Feedback Entries (" + str(len(results)) + ")", entries_inner),
...
def _email_ui(key_suffix, html_fn):
    st.markdown('<div style="margin-top:2.5rem"></div>', unsafe_allow_html=True)
    st.markdown("""
<div style="background:rgba(124,58,237,0.07);border:1px solid rgba(124,58,237,0.22);
border-radius:20px;padding:2rem 2.4rem;position:relative;overflow:hidden;">
  <div style="position:absolute;top:0;left:0;right:0;height:1px;
  background:linear-gradient(90deg,transparent,rgba(167,139,250,0.5),transparent);"></div>
  <div style="font-size:0.6rem;font-weight:700;color:#a78bfa;letter-spacing:0.13em;
  text-transform:uppercase;margin-bottom:0.5rem;">&#128231; Email Report</div>
  <div style="font-size:1rem;font-weight:700;color:#fff;margin-bottom:0.4rem;">
    Receive your analysis report by email</div>
  <div style="font-size:0.84rem;color:#9490b0;margin-bottom:0;">
    Enter your email and we will send the full report with sentiment scores and aspect breakdown.</div>
</div>""", unsafe_allow_html=True)

    sent_key = "_sent_" + key_suffix
    html_key = "_html_" + key_suffix

    st.markdown("<div style='margin-top:0.9rem'></div>", unsafe_allow_html=True)
    ec1, ec2 = st.columns([4, 1], gap="small")
    with ec1:
        to_email = st.text_input("_email_" + key_suffix, placeholder="your@email.com", label_visibility="collapsed")
    with ec2:
        send_btn = st.button("Send →", key="send_btn_" + key_suffix, use_container_width=True)

    # Persist success + download buttons across reruns
    if st.session_state.get(sent_key):
        st.success("✓ Report sent to **" + st.session_state[sent_key] + "**")
        if html_key in st.session_state:
            da, db = st.columns(2, gap="small")
            with da:
                st.download_button(
                    "⬇ Download Report",
                    data=st.session_state[html_key].encode("utf-8"),
                    file_name="sentimentiq_report.html",
                    mime="text/html",
                    key="dl_persist_" + key_suffix,
                    use_container_width=True
                )
            with db:
                st.download_button(
                    "⬇ Download Charts",
                    data=st.session_state.get("_charts_" + key_suffix, b""),
                    file_name="sentimentiq_charts.html",
                    mime="text/html",
                    key="dl_charts_persist_" + key_suffix,
                    use_container_width=True
                )
        ...
        if not to_email or "@" not in to_email or "." not in to_email:
            ...
            with st.spinner("Generating and sending report…"):
                try:
                    ...
                    # Build HTML — catch build errors separately
                    try:
                        html = html_fn()
                    except Exception as build_err:
                        st.error("Report build error: " + str(build_err))
                        st.stop()
                    st.session_state[html_key] = html
                    # Send
                    msg = MIMEMultipart("alternative")
                    msg["Subject"] = "SentimentIQ — Analysis Report"
                    msg["From"]    = "SentimentIQ <" + _SENDER_EMAIL + ">"
                    ...
                    with smtplib.SMTP("smtp.gmail.com", 587, timeout=30) as srv:
                        ...
                        srv.starttls()
                        srv.ehlo()
                        srv.login(_SENDER_EMAIL, _APP_PASSWORD)
                        srv.sendmail(_SENDER_EMAIL, [to_email.strip()], msg.as_string())
                    st.session_state[sent_key] = to_email.strip()
                    st.success("✓ Report sent to **" + to_email + "**")
                    da, db = st.columns(2, gap="small")
                    with da:
                        st.download_button(
                            "⬇ Download Report",
                    ...
                    with db:
                        st.download_button(
                            ...
                            file_name="sentimentiq_charts.html",
                            mime="text/html",
                            key="dl_charts_now_" + key_suffix,
                            use_container_width=True
                        )
                except smtplib.SMTPAuthenticationError:
                    st.error("❌ Authentication failed. Make sure you are using a Gmail App Password.")
                except smtplib.SMTPException as smtp_err:
                    st.error("❌ SMTP error: " + str(smtp_err))
...
<div class="fcard-outer">
  <div class="fcard-body">
    ...
    <div class="fcard-desc">Analyse one piece of feedback in full detail. Get overall sentiment, aspect breakdown, sentiment scores, and probability distributions - instantly.</div>
...
<div class="pghdr">
  ...
  <div class="pghdr-badge">✦ Deep Inspection</div>
    ...
    if (analyze_btn or ex1 or ex2 or ex3) and active_text.strip():
        ...
        st.session_state['single_text']   = active_text
        # Pre-build charts HTML for download
        try:
            _proba = result["probabilities"]
            if _proba:
                _lbls = list(_proba.keys())
                _vals = [v * 100 for v in _proba.values()]
                _fig_c = go.Figure(go.Bar(
                    x=_lbls, y=_vals,
                    marker=dict(color=[SC.get(l,'#a78bfa') for l in _lbls], line=dict(width=0)),
                    text=[f"{v:.1f}%" for v in _vals], textposition='outside',
                    textfont=dict(color='#9490b0'),
                ))
                _fig_c.update_layout(
                    title=dict(text='Probability Distribution', font=dict(color='#edeaf8', size=14)),
                    yaxis=dict(range=[0,118], ticksuffix='%', tickfont=dict(color='#9490b0'),
                               gridcolor='rgba(255,255,255,0.05)'),
                    xaxis=dict(tickfont=dict(color='#9490b0')),
                    height=380, **PT)
                _chart_html = ("<html><head><meta charset='UTF-8'>"
                               "<style>body{background:#05050d;margin:0;padding:20px}</style></head><body>"
                               + _fig_c.to_html(full_html=False, include_plotlyjs='cdn') + "</body></html>")
                st.session_state['_charts_single'] = _chart_html.encode('utf-8')
        except Exception:
            st.session_state['_charts_single'] = b""


        badge_cls  = "badge-" + result['sentiment'].lower()
        score_val  = sent_to_scale(result['confidence'], result['sentiment'])
...
<div class="rbanner">
  ...
  <div class="rgrid">
    ...
    <div class="rtile">
      ...
      <span class="rtile-val" style="color:{sent_color}">{score_val} / 5</span>
    </div>
    <div class="rtile">
...
<div class="acard">
  <div style="flex:1">
    ...
    <div class="acard-score">Score: {sent_to_scale(ar['confidence'],ar['sentiment'])}/5</div>
  ...
  <span class="{bc_}" style="margin-left:1rem;flex-shrink:0">{ar['emoji']} {ar['sentiment']}</span>
        ...
        with rc:
            ...
            if proba:
                ...
                st.download_button(
                    "⬇ Download Chart",
                    data=fig_p.to_html(full_html=True, include_plotlyjs='cdn').encode(),
                    file_name="probability_chart.html",
                    mime="text/html",
                    key="dl_single_chart",
                    use_container_width=True
                )
    ...
    if 'single_result' in st.session_state:
        _r = st.session_state['single_result']
        _email_ui('single', lambda: build_single_html(_r))
...
<div class="bgrid">
  ...
  <div class="bpanel">
        ...
        try:
            ...
            if 'feedback' not in df.columns:
                ...
                if st.button("Run Batch Analysis →", type="primary"):
                    ...
                    # Store uploaded file for email attachment
                    uploaded_file.seek(0)
                    st.session_state['batch_file_bytes'] = uploaded_file.read()
                    st.session_state['batch_file_name']  = uploaded_file.name
                    # Pre-build batch charts for download
                    try:
                        _bstats = compute_summary_stats(batch)
                        _pie_f  = make_pie_chart(_bstats['positive'], _bstats['neutral'], _bstats['negative'])
                        _bar_f  = make_aspect_bar_chart(_bstats['aspect_sentiment']) if _bstats['aspect_sentiment'] else None
                        _cht    = ("<html><head><meta charset='UTF-8'>"
                                   "<style>body{background:#05050d;margin:0;padding:20px}"
                                   "h3{font-family:Helvetica Neue,Arial,sans-serif;color:#a78bfa;"
                                   "font-size:12px;text-transform:uppercase;letter-spacing:0.1em;"
                                   "margin:0 0 10px}</style></head><body>"
                                   "<h3>Sentiment Distribution</h3>"
                                   + _pie_f.to_html(full_html=False, include_plotlyjs='cdn'))
                        if _bar_f:
                            _cht += "<h3 style='margin-top:24px'>Aspect-wise Sentiment</h3>" + _bar_f.to_html(full_html=False, include_plotlyjs=False)
                        _cht += "</body></html>"
                        st.session_state['_charts_batch'] = _cht.encode('utf-8')
                    except Exception:
                        st.session_state['_charts_batch'] = b""
                ...
                if 'batch_results' in st.session_state and st.session_state.batch_results:
                    ...
                    result_df = pd.DataFrame([{
                        ...
                        'Sentiment Score': f"{sent_to_scale(r['confidence'],r['sentiment'])}/5",
                    ...
                    styled_df = result_df.style.map(color_sentiment, subset=['Sentiment'])
                    ...
                    with dl1:
                        st.download_button("⬇ Download CSV", result_df.to_csv(index=False), file_name="absa_results.csv", mime="text/csv", use_container_width=True)
                    with dl2:
                        _pie_fig  = make_pie_chart(stats['positive'], stats['neutral'], stats['negative'])
                        _bar_fig  = make_aspect_bar_chart(stats['aspect_sentiment']) if stats['aspect_sentiment'] else None
                        _chart_html = _pie_fig.to_html(full_html=False, include_plotlyjs='cdn')
                        if _bar_fig:
                            _chart_html += "<br>" + _bar_fig.to_html(full_html=False, include_plotlyjs=False)
                        _chart_page = "<html><head><meta charset='UTF-8'><style>body{{background:#05050d;margin:0;padding:20px}}</style></head><body>" + _chart_html + "</body></html>"
                        st.download_button("⬇ Download Charts", data=_chart_page.encode(), file_name="sentiment_charts.html", mime="text/html", use_container_width=True)
                    with dl3:
                        _report_bytes = build_batch_html(results, stats, st.session_state.get('batch_file_bytes'), st.session_state.get('batch_file_name')).encode()
                        st.download_button("⬇ Download Report", data=_report_bytes, file_name="sentimentiq_report.html", mime="text/html", use_container_width=True)
                    ...
                    for i, r in enumerate(results[:show_n]):
                        ...
                        with st.expander(f"{r['emoji']}  #{i+1}  —  {r['original_text'][:90]}…"):
                            ...
                            with cc3:
                                st.markdown(f'<div style="color:#4c4966;font-size:0.61rem;text-transform:uppercase;letter-spacing:0.1em;font-weight:700;margin-bottom:4px">Score</div><div style="color:#edeaf8;font-size:0.9rem;font-weight:700">{sent_to_scale(r["confidence"],r["sentiment"])}/5</div>', unsafe_allow_html=True)
    ...
    # Email report for batch
    if 'batch_results' in st.session_state and st.session_state.batch_results:
        _br  = st.session_state.batch_results
        _bs  = compute_summary_stats(_br)
        _fbytes = st.session_state.get('batch_file_bytes')
        _fname  = st.session_state.get('batch_file_name')
        _email_ui('batch', lambda: build_batch_html(_br, _bs, _fbytes, _fname))
